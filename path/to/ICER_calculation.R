...
############ Jan 2 2021
# Function code to calculate base ICER for different number of years since initiation 

ICER_base<- function(numyears,filename){

  Rate = 110
  dis_rate = 0.02
  Simulation_total_control <- c()
  ...
  (Simulation_cost_total <- Simulation_cost_NoPrEP + Simulation_cost_PrEP)
  (Simulation_qaly_total <- Simulation_qaly_NoPrEP + Simulation_qaly_PrEP)
  
  (Cost_and_QALY <- c(Simulation_cost_total,Simulation_qaly_total))
  ...
  (Simulation_total_control <- c(Simulation_cost_control,Simulation_qaly_control))
  Difference_cost <- Simulation_cost_total - Simulation_cost_control
  Difference_qaly <- Simulation_qaly_total - Simulation_qaly_control
  (ICER <- Difference_cost/Difference_qaly)
  dir.create(filename, showWarnings = FALSE)
  write.csv(x = ICER,paste0(filename,"/ICER_results_",numyears,".csv"))
}
